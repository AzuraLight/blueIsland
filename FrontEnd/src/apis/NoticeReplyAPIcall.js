...
export const callReplyRegistAPI = ({ form, boardNo, noticeNo }) => {
  ...
  const requestURL = `http://${process.env.REACT_APP_RESTAPI_IP}:8090/api/notices/replies/user-only`;
...
// 댓글 전체 조회 함수
export const callReplyListAPI = ({ noticeNo, memberId }) => {
  const requestURL = `http://${process.env.REACT_APP_RESTAPI_IP}:8090/api/notices/replies/read-only/${noticeNo}`;
...
// 댓글 수정 함수
export const callReplyUpdateAPI = ({ form, replyNo, noticeNo }) => {
  console.log("[ReplyAPICalls] callReplyUpdateAPI Call");

  const requestURL = `http://${process.env.REACT_APP_RESTAPI_IP}:8090/api/boards/replies/user-only/${noticeNo}/${replyNo}`;
...
// 댓글 삭제 함수
export const callReplyDeleteAPI = ({ noticeNo, replyNo }) => {
