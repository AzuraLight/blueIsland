...
import com.uni.blueisland.jwt.JwtAccessDeniedHandler;
import com.uni.blueisland.jwt.JwtAuthenticationEntryPoint;
...
//    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable()
                ...
                .antMatchers("/api/notices/admin-only", "/api/notices/admin-only/**").hasRole("ADMIN")
                .antMatchers("/api/notices/read-only","/api/notices/read-only/**").permitAll()
                .antMatchers("/api/boards/user-only", "/api/boards/user-only/**").hasAnyRole("USER", "ADMIN")
                .antMatchers("/api/boards/read-only","/api/boards/read-only/**").permitAll()
...
//         configuration.setAllowedOrigins(Arrays.asList("http://localhost:3000" ));// 해당 ip만 응답
        configuration.setAllowedOrigins(Arrays.asList("http://192.168.0.152:3000" ));// 해당 ip만 응답 - 도커
